    ...
    */
    ...
    case 0x01: // CONNECT
    {
        co_spawn(
            ...
            std::bind(&Session::cross_two_tcp, shared_from_this()),
            detached);
    }
    break;
    case 0x02: // BIND
...
awaitable<void> Session::cross_two_tcp()
{
    auto self(shared_from_this());
    for (;;)
    {
        auto [ec, n] = co_await in_socket_.async_read_some(asio::buffer(in_buf_), as_tuple(use_awaitable));
        if(ec) break;
        tie(ec, n) = co_await asio::async_write(out_socket_, asio::buffer(in_buf_, n), as_tuple(use_awaitable));
        if(ec) break;
        tie(ec, n) = co_await out_socket_.async_read_some(asio::buffer(out_buf_), as_tuple(use_awaitable));
        if(ec) break;
        tie(ec, n) = co_await asio::async_write(in_socket_, asio::buffer(out_buf_, n), as_tuple(use_awaitable));
        if(ec) break;
    }
}
awaitable<std::tuple<bool, udp::resolver::results_type, uint8_t>> Session::resolve_udp_target_addr()
{
    ...
    std::tie(ec, eps) = co_await resolver.async_resolve(
        ...
        udp_r_port,
        as_tuple(use_awaitable));
    if(ec) co_return std::make_tuple(false, eps, skip_n);
